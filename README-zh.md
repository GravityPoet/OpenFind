...
**一个 App，5 个创造性的工具**  
*毫秒级深层搜索、全加密 OCR 剪贴板、合盖防休眠、外接磁盘助手与键盘锁。*

OpenFind 让你不再被多工具卡顿折磨：**零延迟穿透代码与 ZIP 包**、**复制截图瞬间提取文字且全加密防护**、**合上 MacBook 屏幕编译与下载绝不断开**、**按需查看外接磁盘状态并单次唤醒**。

[**🚀 立即下载 OpenFind v1.1.4 (macOS Universal)**](https://github.com/GravityPoet/OpenFind/releases/tag/v1.1.4) ·
[查看 v1.1.4 发布说明](https://github.com/GravityPoet/OpenFind/releases/tag/v1.1.4)
...
| **外接硬盘健康** | 🛑 移动硬盘频繁休眠、卡死 Finder 或自动断开。 | 💾 **外接磁盘助手**：按文件夹显示状态并单次唤醒；可选的高级持续模式仅可能减少部分机械盘的唤醒等待。 |
...
### 1. ⚡ 极速搜索引擎 (文件与全文)
* **快速搜索**：在同一面板查找应用、文件、系统设置、Safari/Chrome 书签（含 Safari 阅读列表）和已授权的联系人，支持中英文与拼音。`⌘1–9` 打开前九项，其余结果可加载更多；底部「搜索模式」可直接选择各类入口。
* **全量搜索**：保留文件正文、正则、标签、大小、日期、搜索范围、预览和全部结果。快速搜索的 `in 关键词`、`tags 标签` 或 `⌥↩` 可直接转入。
...
* **配置迁移与同步**：在「设置 → 搜索 → 配置与同步」导出、导入可迁移偏好和保存的文本片段，或选择同步文件夹。剪贴板历史、密钥、权限、登录项、文件夹授权和硬盘目标保留在各自 Mac；冲突会保留双方副本，导入后可恢复上一次配置。JSON 包含可读片段，请选择私人文件夹。
...
### 4. 💾 外接磁盘助手
* **按需状态与唤醒**：按文件夹显示连接与可写状态，每个目标可执行一次轻量唤醒，不启动后台循环。
* **高级持续模式（可选）**：仅在显式开启后按间隔覆盖一个有上限的小标记文件，会增加耗电，不能修复线缆、供电、扩展坞、驱动、网络或固件导致的断连。
...
```text
# --- 快速搜索模式（命令后加空格）---
app Safari             # 只搜索应用；settings 蓝牙 只搜索系统设置
open Report-*.pdf      # 打开文件；前导空格也可进入文件模式
find Report            # 在 Finder 中定位
in 季度预算            # 进入全量搜索的内容检索
bm Swift               # 书签；contacts 张三 搜索联系人
recent TextEdit        # 选择应用，查看它的最近文档
~/Documents/*.pdf      # 当前目录导航与通配符
calc (2+3)*4           # 计算并按回车复制；普通算式也可直接输入
...
⌘↩                     # 在 Finder 中显示选中的结果
⌥↩                     # 从快速搜索进入全量搜索
Tab / →                # 进入选中目录，或查看选中应用的最近文档
⇧Tab / ⌘←              # 返回上层
⌘Y（快速搜索）          # 预览当前文件；↑↓ 选择结果后也可按空格
Space (全量搜索结果)     # 调起原生 Quick Look 快速预览
```

联系人首次使用需在 `contacts ` 模式授权；Safari 书签和部分最近文档需要完全磁盘访问权限。最近文档取决于应用是否向 macOS 保存记录，不会把“该应用能打开的文件”冒充为最近使用的文档。锁屏需要辅助功能权限。

终端命令前缀默认是 `g`，可在“设置 → 搜索 → 在终端中运行”中选择终端并改为任一英文字母。空搜索框会显示当前字母的可点击提示，点击后自动填入前缀并把光标放在空格后。默认目标跟随 macOS 默认终端，也可以在设置中明确选择终端.app 或 Ghostty。命令交给所选终端的 shell 执行，沿用其新窗口或新标签页规则；需要指定目录时输入 `g cd /目标目录 && 命令`。OpenFind 不保存命令历史，所选终端的 shell 历史仍按你的 shell 设置处理。
...
* **本地检索**：零追踪、零云端分析；书签、联系人和最近文档只在本机读取，不进入配置同步。网页搜索仅在你选中并打开后交给浏览器；Sparkle 更新检查可关闭。
...
官方 v1.1.4 客户包同时支持 `arm64` 与 `x86_64`，并使用 OpenFind 固定客户
