...
| 2026-07-24 | Exact-SHA CI run `30082103127` | All three Drive Alive writer checks hit their two-second deadline on the hosted runner before the low-priority file-system work completed; the same three checks passed locally, and serialization removed every unrelated failure | Use one explicit ten-second Drive Alive deadline in the writer, controller, and semantic tests so slow external or hosted disks can complete while still remaining bounded | File-system integration deadlines must reflect slow mounted-disk behavior; semantic tests should share the production deadline instead of imposing a tighter scheduler benchmark |
