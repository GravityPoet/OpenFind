...
| 2026-08-27 | Exact-SHA CI run `33026368321` | A SearchViewModel quiet-period test checked `isSearching` after one 80 ms sleep; the hosted MainActor resumed the test before the scheduled refresh task | Replace the fixed sleep with a bounded state-polling loop that yields to the actor | Use state-based waits for every asynchronous refresh/timer assertion, even when the nominal delay is short |
