...
| 2026-07-24 | Exact-SHA CI run `30083105936` | The first Drive Alive semantic fixture remained blocked in the hosted virtual disk's real `fsync` for ten seconds; that non-cancellable utility work then starved the two conflict fixtures and a later content-index check | Keep real bounded `fsync` in production, inject a deterministic sync boundary into semantic writer tests, and add a separate failure-mapping test | Unit semantics must isolate blocking OS durability calls; validate the production syscall in packaged smoke/integration checks rather than using shared-runner latency as an assertion |
