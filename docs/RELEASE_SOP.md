...
| 2026-08-31 | Continue watching exact-SHA CI run `33341803996` through the local `gh run watch` process session | The local observation session had already closed, so polling it returned `write_stdin failed: Unknown process id 88878` even though GitHub CI had completed successfully | Query `gh run view 33341803996 --json status,conclusion,jobs,url,headSha` and verify the exact SHA and every job directly | Treat a closed local watcher as an observation-channel failure, not a CI result; fall back immediately to a direct GitHub run query and do not restart the workflow |
| 2026-08-31 | Compare the public `v1.1.3` Release body byte-for-byte with `docs/releases/v1.1.3.md` | `diff -u` reported one extra blank line at EOF in the body returned by `gh release view --json body`, although all release content matched | Normalize trailing blank lines for the comparison and separately assert the required bilingual sections, links, version, and build fields | GitHub Release API may append a terminal newline; compare normalized Markdown content rather than raw EOF bytes |
| 2026-08-31 | Inspect the public `v1.1.3` appcast against the SOP's three-release retention statement | The downloaded feed contained only `sparkle:version 1001003`; the `v1.1.0`–`v1.1.2` feeds show the same single-entry behavior because the workflow generates each feed in a fresh directory | Accept the current signed latest-version enclosure for this release and schedule an appcast merge/retention gate before the next release; do not claim three-version retention without counting entries | Seed `generate_appcast` with verified prior public appcast entries (or revise the contract) and assert the expected entry count before publication |
| 2026-09-24 | Push the new `v1.1.4` tag with `git push origin v1.1.4` | The first push failed with `Connection closed by 20.27.177.118 port 443`; the same transient had already interrupted one preflight `git fetch` | Verified the local tag object existed (`git rev-parse "v1.1.4^{commit}"`), waited, and retried the identical push, which reported `[new tag] v1.1.4 -> v1.1.4` | Treat SSH transport resets as retriable; always confirm local tag state before retrying and never recreate or move the tag to recover a transport failure |
| 2026-09-24 | List release secrets with `gh secret list` during `v1.1.4` preflight | The API returned `failed to get secrets: ... EOF` twice while `gh auth status` and `gh release view` worked, so it was a transport flake on that endpoint, not an auth loss | Recorded the limitation and proceeded; the Release workflow's pinned-certificate import step passing on the exact tag SHA serves as secret-presence evidence | Keep secret-endpoint transport errors separate from auth conclusions; require the workflow-owned cert/secret steps to pass rather than blocking on a local secret listing |
| 2026-09-24 | Download public `v1.1.4` assets with `gh release download` from inside the clean temp directory | `gh` failed with `fatal: not a git repository` because the working directory had changed to the download target | Re-ran from the repository root with `gh release download v1.1.4 --repo GravityPoet/OpenFind --dir <tmp> --clobber` | Never change out of the repository before `gh release download`; always pass `--repo` and `--dir` explicitly for clean-directory public-asset verification |
